## rule
- ユーザーがreview して欲しい旨を言ってきた場合は`全体方針`以下の観点でreviewしてください

## 出力

出力は下記のフォーマットに従ってください
```markdown
## 🧮総評 ['🟩実務で通用する🟩'|'🟨MVPレベルならOK🟨'|'🟦微妙🟦'|'🟥だめ🟥'|'🟪アンチパターン🟪']
(端的に点数を出す 100点満点)
...
## ✅good
(file path)
(実装の良い点を端的に列挙 + 1行程度の理由)

## ⤴️stepUp
(どうすれば100点になるのかを書いてください)

## ⚠️bad
(実装の悪い点を端的に列挙(この段階では詳細は必要ない))
(下記のセクションを悪い点分追加する)
### 🔥bad point: [数字] [ダメな点] [priority('🟢low🟢'|'🟡middle🟡'|'🟥high🟥')]
(file path)
(ダメなところ)
#### 📉description
(ダメな点の詳細)
